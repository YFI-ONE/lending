...
contract UniswapAnchoredView is UniswapConfig {
    ...
    function priceInternal(TokenConfig memory config) internal view returns (uint) {
        ...
        if (config.priceSource == PriceSource.FIXED_BNB) {
            uint usdPerEth = prices[ethHash];
            require(usdPerEth > 0, "BNB price not set, cannot convert to dollars");
     ...
     * @dev Implements the PriceOracle interface for Yfione v2.
    ...
    function fetchEthPrice() internal returns (uint) {
        return fetchAnchorPrice("BNB", getTokenConfigBySymbolHash(ethHash), ethBaseUnit);
    }

    /**
     * @dev Fetches the current token/usd price from uniswap, with 6 decimals of precision.
     * @param conversionFactor 1e18 if seeking the BNB price, and a 6 decimal BNB-USDC price in the case of other assets
     */
    function fetchAnchorPrice(string memory symbol, TokenConfig memory config, uint conversionFactor) internal virtual returns (uint) {
        ...
        // Adjust rawUniswapPrice according to the units of the non-BNB asset
        // In the case of BNB, we would have to scale by 1e6 / USDC_UNITS, but since baseUnit2 is 1e6 (USDC), it cancels
        if (config.isUniswapReversed) {
